...
class RejectPaymentImportHandler implements CommandHandlerInterface
{
    ...
    public function __invoke(RejectPaymentImport $command): void
    {
        $payment = $this->paymentRepository->load($command->paymentId);
        $payment->rejectImport($this->clock->now());
